...
IMAGENS = {
    ...
    "JPG → PNG": (jpg_to_png, ".png", [("Imagens JPG", "*.jpg"), ("Imagens JPEG", "*.jpeg")]),
...
class ConverterApp(tk.Tk):
    def __init__(self):
        ...
        self.btn_selecionar = ttk.Button(
            frame_controles, text="📂 Selecionar Arquivos", command=self.selecionar_arquivos, width=18
        )
        self.btn_converter = ttk.Button(
            frame_controles, text="⚡ Converter", command=self.converter_arquivos, width=14
        )
        ...
        lbl_categoria = tk.Label(frame_combobox, text="Categoria:", bg="#ffffff", font=("Segoe UI", 11))
        ...
        lbl_conversao = tk.Label(frame_combobox, text="Conversão:", bg="#ffffff", font=("Segoe UI", 11))
    ...
    def selecionar_arquivos(self):
        ...
        _, _, tipos = FUNCOES_CONVERSAO.get(option, (None, None, [("Todos os arquivos", "*.*")]))
    ...
    def converter_arquivos(self):
        ...
        option = self.conversao_var.get()
        funcao_converter, extensao_saida, _ = FUNCOES_CONVERSAO.get(option, (None, None, None))
        ...
        for i, (nome, caminho_entrada) in enumerate(self.arquivos_dict.items()):
            ...
            if option == "PDF → PNG":
                caminho_saida = pasta_saida  # apenas a pasta, pois gera múltiplos arquivos
            else:
                caminho_saida = os.path.join(pasta_saida, f"{nome_base}{extensao_saida}")
